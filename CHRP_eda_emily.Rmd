---
title: "Analysis of Case and Death Count Growth Rates in Texas Counties"
...
Examining factors impacting growth rates in fatalities and case counts and the discrepancy between the two may inform bias correction for reporting in local communities.
...
Case and fatality counts both exhibit exponential growth, which can be quantified using the rate of change for the log of the counts. Regressing the logged counts against dates gives an average growth rate for each of the case and fatality curves. Below are the calculated growth rates for Texas counties for which there is non-zero data.
...
After calculating the case and fatality growth rates for all 254 Texas counties, datasets containing socioeconomic data such as population demographics, income level, and poverty rates was found for each county. This information was then used in a regression analysis to identify which were most influential on the case and fatality growth rates.
...
After experimentation the following models were found to best explain county-to-county variation in case count and fatality count growth rates.
...
The growth rate analysis and lag analysis would both benefit from a time-series approach as the pandemic progresses to advanced phases. As seen on the plot for Texas case and fatality counts, the growth rate has decreased over the last several days. Time-series modeling would better capture these changes, as well as when they occur.
